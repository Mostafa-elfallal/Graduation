...
void USART1_IRQHandler(void)
{
  ...
  * 4 --> found FESC
  * * * * * * * * * * * * * * * * * * * * * * * * * */
  Frame_t * pFrame = FIFO_getCurrent(&FIFO_IN,FIFO_IN.rear); // get the upcoming frame
  ...
  {   // recieved data ?
    ...
    {
      // this is ending FEND 
      //FIFO_queue(&FIFO_IN,c);
      // complete the frame and enqueue it
      if( isthisme != 2 && (!FIFO_isFull(&FIFO_IN)))
      {
        FRAME_changeState(pFrame,sFRAME_RECEIVED);
        FIFO_enqueue(&FIFO_IN);
      }
      isthisme = 0;
      return;
    }
    ...
    case 1:
      ...
      if(c == OBC_ADDRESS)
      {
        //FIFO_queue(&FIFO_IN,ADCS_ADDRESS);
        //FIFO_queue(&FIFO_IN,OBC_ADDRESS);
        FRAME_put(pFrame , c);
        isthisme = 3;
      }
      else {
    ...
    case 4:
      if( c == 0xdc ){
        //FESC TFEND
        FRAME_put(pFrame , 0xc0);
        isthisme = 3;
      }
      else if ( c == 0xdd ){
        //FESC TFESC
        FRAME_put(pFrame , 0xdb);
        isthisme = 3;
      }
      else 
        //  error 
