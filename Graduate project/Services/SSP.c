...
typedef void (*pfCommand)(Frame_t * Framein , Frame_t * Frameout);

static const pfCommand RecievedType[]={
  ...
  (pfCommand)&CUSTOM,           (pfCommand)&CUSTOM,         (pfCommand)&CUSTOM,    
  (pfCommand)&CUSTOM,           (pfCommand)&CUSTOM,         (pfCommand)&CUSTOM,    
  (pfCommand)&CUSTOM,           (pfCommand)&addrt,          (pfCommand)&delrt,
  (pfCommand)&getrt,            (pfCommand)&RESERVED,       (pfCommand)&RESERVED,  
...
uint8_t FRAME_checkError    (Frame_t* me,error_t newerror)
{
...
{
    Frame->data[Frame->datacnt++] = item;
}
  // marks the frame as completed : got a FEND after it 
...
{
  return (Frame->datacnt >= 5); // minimum number which is ( dest srce type crc0 crc1)
}  
  

void FRAME_typeCall(uint8_t type , Frame_t* framein , Frame_t* frameout){
  RecievedType[type](framein,frameout);
}

//checks if the recieved type is defined or not
uint8_t FRAME_typeCheck(Frame_t* me)
{
  uint8_t type =me->data[TYPE];
  if(type<=63)
    if(RecievedType[type]!= &CUSTOM && RecievedType[type]!= &RESERVED)
      return 1;
  return 0;
}

//compare the recieved CRC with calculated CRC
// used with the input SSP
uint8_t  FRAME_CRCCmp(Frame_t* me,uint16_t crc){
  uint16_t rcvCRC = (uint16_t)((uint16_t)(me->data[me->datacnt-1]) << 8) 
                                | (uint16_t)me->data[me->datacnt-2];
  return (crc == rcvCRC);
}

// puts the CRC value in its place 
// used with output SSP
void FRAME_CRCSet(Frame_t* me,uint16_t crc)
{
  FRAME_put(me,(uint8_t)(crc&0xff));
  FRAME_put(me,(uint8_t)((crc >> 8) & 0xff));
}
void FRAME_send(Frame_t* me)
{
  
  USART_SendData(  USART1 , 0xc0 );
  USART_SendStr(USART1,me->data,me->datacnt); // this takes care of FESCTFEND & FESCTFESC
  USART_SendData(  USART1 , 0xc0 );
}
// let's do some calculations
uint16_t FRAME_CRCCalc(Frame_t * me , uint8_t difflen){
  uint16_t crc = 0xffff; uint8_t *bufp = me->data;
  uint8_t len = me->datacnt - difflen;
  ...
  for (; len > 0; len--)
    crc = (crc >> 8) ^ (uint16_t)bytetab[(crc & 0xff) ^ *bufp++]; 
  return crc;
}